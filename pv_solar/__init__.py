...
# load data to be used in this package
pkg = importlib_resources.files("pv_solar")
my_file0 = pkg / "data" / "coeff0y.npy"
my_file1 = pkg / "data" / "coeff0.npy"
my_file2 = pkg / "data" / "delta_t.npy"
my_file3 = pkg / "data" / "data_iut.pkl"
iut_data = pd.read_pickle(my_file3)
...
def nutation_long_obl(jce):
    ...
    """
    # nut2pd2terms = np.load("coeff0y.npy", allow_pickle=True)
...
def earth_helio_all(jme):
    ...
    """
    # earth0data = np.load("coeff0.npy", allow_pickle=True)
...
def earth_dlt_t(time_step, fig=False):
    ...
    # delta_t = np.load("delta_t.npy")  # load delta_t data
...
def bird_clear_sky(time_step, offset, φ, σ, pssr, oz, wtr, τ500, τ380, ba, rg):
    ...
    pandas.DataFrame
        ...
        * Direct pv_solar irradiance on a horizontal surface (W/m²)
        * Solar irradiance on a horizontal surface from scattered light (W/m²)
        * Total (global) pv_solar irradiance on a horizontal surface (W/m²)
        * Day of year and hour in decimal
        * Diffuse pv_solar irradiance on a horizontal surface (W/m²)
    """
